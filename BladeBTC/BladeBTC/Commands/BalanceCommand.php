	...
	public function handle($arguments)
	{
		...
		if ($user->exist() == false) {
			...
			if (count($investment) > 0) {
				...
				$investment_data = "Non hai ancora investito" . InvestmentPlan::getValueByName("minimum_invest") . " BTC";
			}
...
<b>Balance</b>
" . Btc::Format($user->getBalance()) . " BTC ( $ " . Btc::FormatUSD($user->getBalance()) . " EUR )\n
<b>Invested</b>
" . Btc::Format($user->getInvested()) . " BTC ( $ " . Btc::FormatUSD($user->getInvested()) . " EUR )\n
<b>Reinvested</b>
" . Btc::Format($user->getReinvested()) . " BTC ( $ " . Btc::FormatUSD($user->getReinvested()) . " EUR )\n
<b>Profit</b>
" . Btc::Format($user->getProfit()) . " BTC ( $ " . Btc::FormatUSD($user->getProfit()) . " EUR )\n
<b>Payout</b>
" . Btc::Format($user->getPayout()) . " BTC ( $ " . Btc::FormatUSD($user->getPayout()) . " EUR )\n
<b>Commission</b>
" . Btc::Format($user->getCommission()) . " BTC ( $ " . Btc::FormatUSD($user->getCommission()) . " EUR )\n
<b>Deposit (Confirmed)</b>
" . Btc::Format($user->getLastConfirmed()) . " BTC ( $ " . Btc::FormatUSD($user->getLastConfirmed()) . " EUR )\n
<b>Deposit (Lower than minimum invest)</b>
" . Btc::Format($user->getLastConfirmed() - $user->getInvested()) . " BTC ( $ " . Btc::FormatUSD($user->getLastConfirmed() - $user->getInvested()) . " EUR )\n
...
" . Btc::Format(Investment::getActiveInvestmentTotal($user->getTelegramId())) . " BTC ( $ " . Btc::FormatUSD(Investment::getActiveInvestmentTotal($user->getTelegramId())) . " USD )",
            ...
            $this->replyWithMessage([
                'text'         => "
\xF0\x9F\x95\xA5 <b>I tuoi investimenti</b> \xF0\x9F\x95\xA5
" . $investment_data,
                'reply_markup' => $reply_markup,
                'parse_mode'   => 'HTML',
            ]);
